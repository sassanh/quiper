...
struct SettingsServiceTests {
    ...
    @Test func engineSettingsShortcut_NewUserDefaultsEnabledWithoutMigrationPrompt() {
        Settings.shared.wipeAllData()
        _ = Settings.shared.loadSettings()
        defer { Settings.shared.wipeAllData() }

        #expect(Settings.shared.needsEngineSettingsShortcutMigrationPrompt == false)
        #expect(Settings.shared.customActions.contains { $0.name == "Settings" })
        #expect(Settings.shared.makePersistedSettings().didResolveEngineSettingsShortcutMigration == nil)
    }

    @Test func engineSettingsShortcut_MissingActionTriggersMigrationAndAddResolveInstalls() {
        Settings.shared.wipeAllData()
        _ = Settings.shared.loadSettings()
        defer { Settings.shared.wipeAllData() }
        ...
        #expect(Settings.shared.needsEngineSettingsShortcutMigrationPrompt == true)
        #expect(!Settings.shared.customActions.contains { $0.name == "Settings" })
        #expect(Settings.shared.makePersistedSettings().didResolveEngineSettingsShortcutMigration == nil)

        Settings.shared.resolveEngineSettingsShortcutMigration(add: true)
        #expect(Settings.shared.needsEngineSettingsShortcutMigrationPrompt == false)
        #expect(Settings.shared.customActions.contains { $0.name == "Settings" })

        Settings.shared.resolveEngineSettingsShortcutMigration(add: true)
        #expect(Settings.shared.customActions.filter { $0.name == "Settings" }.count == 1)
        #expect(Settings.shared.makePersistedSettings().didResolveEngineSettingsShortcutMigration == true)
    }

    @Test func engineSettingsShortcut_DeclineResolvePersistsWithoutInstalling() {
        Settings.shared.wipeAllData()
        _ = Settings.shared.loadSettings()
        defer { Settings.shared.wipeAllData() }
        ...
        Settings.shared.resolveEngineSettingsShortcutMigration(add: false)
        #expect(Settings.shared.needsEngineSettingsShortcutMigrationPrompt == false)
        #expect(!Settings.shared.customActions.contains { $0.name == "Settings" })
        #expect(Settings.shared.makePersistedSettings().didResolveEngineSettingsShortcutMigration == false)

        // Re-applying the resolved payload must not re-prompt.
        Settings.shared.applyPersistedSettings(Settings.shared.makePersistedSettings())
        #expect(Settings.shared.needsEngineSettingsShortcutMigrationPrompt == false)
        #expect(!Settings.shared.customActions.contains { $0.name == "Settings" })
    }

    @Test func engineSettingsShortcut_AlreadyInstalledNeedsNoPrompt() {
        Settings.shared.wipeAllData()
        _ = Settings.shared.loadSettings()
        defer { Settings.shared.wipeAllData() }
        ...
        let imported = PersistedSettings(
            services: Settings.shared.services,
            customActions: Settings.shared.defaultActionTemplates,
            didResolveEngineSettingsShortcutMigration: nil,
            quiperVersion: "4.4.1"
        )
        Settings.shared.applyPersistedSettings(imported)
    ...
    @Test func engineSettingsShortcut_DoubleLoadDoesNotStampMarkerOrClearPrompt() {
        Settings.shared.wipeAllData()
        defer { Settings.shared.wipeAllData() }

        // Simulate an existing install that predates the Settings action (key absent).
        let preFeature = PersistedSettings(
            services: [Service(name: "Gemini", url: "https://example.com", focus_selector: "")],
            hasCompletedGhostOnboarding: true,
            enableHUDDoubleTapCmd: true,
        ...
        // didSet-driven saves during apply/load must not stamp the key while migration is pending.
        Settings.shared.saveSettings()
        #expect(Settings.shared.makePersistedSettings().didResolveEngineSettingsShortcutMigration == nil)
    ...
    @Test func engineSettingsShortcut_NewerSettingsDeferMigrationAndPreserveVersion() {
        Settings.shared.wipeAllData()
        _ = Settings.shared.loadSettings()
        defer { Settings.shared.wipeAllData() }
        ...
        let persisted = Settings.shared.makePersistedSettings()
        #expect(Settings.shared.needsEngineSettingsShortcutMigrationPrompt == false)
        #expect(persisted.didResolveEngineSettingsShortcutMigration == nil)
        #expect(persisted.quiperVersion == futureVersion)
    }

    @Test func engineSettingsShortcut_UnknownSettingsVersionDefersConservatively() {
        Settings.shared.wipeAllData()
        _ = Settings.shared.loadSettings()
        defer { Settings.shared.wipeAllData() }
