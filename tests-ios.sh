...
app_bundle=$(find "$DERIVED_DATA_PATH/Build/Products" \
    -path "*/QuiperiOS.app" -type d -print -quit)
if [[ -z "$app_bundle" ]]; then
    echo "QuiperiOS app bundle not found in $DERIVED_DATA_PATH" >&2
    exit 1
fi

app_binary=""
for candidate in "$app_bundle/QuiperiOS.debug.dylib" "$app_bundle/QuiperiOS"; do
...
if [[ -z "$app_binary" ]]; then
    echo "Instrumented QuiperiOS binary not found in $app_bundle" >&2
    exit 1
fi

echo "Exporting coverage from: $app_binary"
