...
final class WebNotificationBridge: NSObject {
    ...
    private func syncInitialPermissionState() {
        UNUserNotificationCenter.current().getNotificationSettings { [weak self] settings in
            ...
            Task { @MainActor [weak self] in
                ...
                NSLog("[Quiper] syncInitialPermissionState status=\(status.rawValue)")
    ...
    private func requestPermission(requestId: Int) {
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .badge, .sound]) { [weak self] granted, error in
            ...
            Task { @MainActor [weak self] in
                guard let self else { return }
                NSLog("[Quiper] requestAuthorization result granted=\(granted) error=\(errorDescription ?? "none")")
                let state: String
    ...
    private func scheduleNotification(title: String, options: [String: Any]) {
        ...
        UNUserNotificationCenter.current().getNotificationSettings { [weak self] settings in
            ...
            Task { @MainActor [weak self] in
                guard let self else { return }
                NSLog("[Quiper] scheduleNotification authorization status=\(status.rawValue)")
                guard Self.isAuthorized(status: status) else {
                    NSLog("[Quiper] scheduleNotification blocked because authorization status=\(status.rawValue)")
                    return
                }
