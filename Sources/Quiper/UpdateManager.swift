...
final class UpdateManager: NSObject, ObservableObject {
    ...
    func handleLaunchIfNeeded() {
        guard settings.updatePreferences.automaticallyChecksForUpdates else { return }
        let now = Date()
    ...
    private func installDownloadedRelease(from fileURL: URL, release: ReleaseInfo) {
        ...
        let installTask = Task.detached(priority: nil) { () throws -> URL in
        ...
        Task { @MainActor [weak self] in
            do {
                let installedURL = try await installTask.value
