...
- **Ephemeral Tabs for Private Browsing**: `Cmd+P` now converts the current tab into an isolated tab in place — and back again — loading the session's address either way. Nothing private ever saves anything — no history, input, or titles reach disk. Engine shortcuts stay disabled inside it to protect your privacy, and a dashed window outline plus a title badge mark the mode. Ephemeral loads also drop Quiper's referral marker from engine URLs.
...
- **Suggest Selector**: Right-clicking the page title or any page element now offers Suggest Selector, sitting right after Inspect. Hovering highlights elements, and picking one opens a dialog with a robust CSS selector, a Root-to-Leaf specificity slider with a live preview and match count, Copy, and Hide — which adds a `display: none` rule to that engine's stylesheet. Ephemeral tabs stay excluded from every menu.
- **Link Right-Click Menu**: Right-clicking a link now offers Open Link Here, Open Link in New Window, Open Link in System Browser, and Open Private instead of the system defaults. Open Private converts the current tab into a private tab and opens the link in it.
- **Pinned-Tab Engines**: Engines can now pin a fixed URL to each tab. Every tab always opens its pinned address — closing a tab never loses it — and links never navigate a pinned tab in place; they open in a popup or the system browser instead. Switch an engine's type in its settings to convert it, and manage all ten tab URLs (with reorder) in the new Pinned Tabs pane. Tabs without a URL stay out of the session selector. In the Tab URLs list, defined tabs are highlighted, empty slots are muted, and reordering animates.
...
- **Return to Previous App from Element Fullscreen**: Pressing the global Show/Hide Quiper shortcut while an HTML element is fullscreen in its dedicated Space now returns focus to the app that was active before Quiper, leaving the element fullscreen in its Space. If there is no previous app to return to, Quiper shows its fullscreen banner instead.
...
- **iOS Update Default Action Scripts Prompt**: Existing installs that predate template script syncing now get the same one-time choice as the Mac: reconnect template-matching actions to Quiper's bundled scripts and keep them synced automatically, or keep them custom and editable.
...
- **iOS Custom CSS Styling**: Each engine's custom CSS now applies to its pages on iOS, matching the Mac app, including the bundled engine templates' default styles.
- **iOS Camera and Microphone Permissions**: When a website requests camera or microphone access on iOS, Quiper asks for the matching system permission and only grants the page if the user allows it, matching the Mac app.
- **iOS Find in Page**: The Actions menu on iOS now offers Find in Page, which opens a find bar in place of the bottom bar. It highlights matches as you type, steps between them with the previous/next buttons, and clears with the in-field clear button—the same matching logic as the Mac app.
...
- **Expanded Selector Placement**: Expanded engine and session selectors now follow the window when its position or size changes and remain fully within the active screen's visible area.
...
- **Prompt Input and Custom CSS Template Sync ([Settings.swift](Quiper/Settings.swift), [SettingsView.swift](Quiper/SettingsView.swift), [WebViewManager.swift](Quiper/Components/WebViewManager.swift))**: Added a per-engine **Use Latest Default** switch for template-backed prompt input selectors and custom stylesheets. Synced resources automatically follow the latest values bundled with Quiper and remain read-only; disabling sync creates an editable local copy of the current default. New bundled engines start in sync, while existing saved engines keep their current custom values until sync is enabled.
- **Spotlight Privacy for Secured Engines ([SpotlightExclusion.swift](Quiper/Components/SpotlightExclusion.swift), [EncryptedVolumeManager.swift](Quiper/Components/EncryptedVolumeManager.swift))**: Secured engine storage is now excluded from Spotlight indexing when volumes are created or mounted, with existing secured engines refreshed automatically at launch.

### Changed

- **Reproducible Settings Editor Builds**: Xcode and CI now rebuild the bundled settings editor from locked dependencies, and CI rejects stale generated assets.
...
- **Prompt Input Naming ([SettingsView.swift](Quiper/SettingsView.swift), [engines.md](docs/engines.md))**: Renamed the former **Prompt Element** settings section to **Prompt Input** and clarified its selector guidance to better describe the editable field Quiper targets and focuses.
- **Nightly Release Delivery ([integration_delivery.yml](.github/workflows/integration_delivery.yml), [build-app.sh](build-app.sh))**: Nightly releases now publish immediately when `main` changes, periodically rebuild after 72 hours, skip unnecessary scheduled builds, derive versions from the latest stable release, and remove superseded nightly releases.

### Fixed

- **Unified HUD List Selection ([PromptHistoryHUDView.swift](Quiper/Components/PromptHistoryHUDView.swift), [ModifierHUDView.swift](Quiper/Components/ModifierHUDView.swift))**: Hovering a Prompt History or Control Center list item now moves the actual keyboard selection to that item, so Enter, `Cmd+C`, and other selection actions consistently target the visibly highlighted row.
- **Compact Selector Expansion Reliability ([CollapsibleSelector.swift](Quiper/CollapsibleSelector.swift))**: Prevented session and engine selector panels from becoming permanently stuck open during rapid collapse and re-expansion. Expansion state now comes directly from the selector-owned panel attached to the main window, while collapsing detaches that ownership before the fade animation so later expansions cannot be cleared by stale animation completion work.
...
- **Control Center Focus Restoration ([ModifierHUDView.swift](Quiper/Components/ModifierHUDView.swift), [MainWindowController+Actions.swift](Quiper/MainWindowController+Actions.swift), [MainWindowController+InputHandling.swift](Quiper/MainWindowController+InputHandling.swift))**: Fixed keyboard focus becoming unreliable after repeatedly opening and closing Control Center, including when it is dismissed with Escape or used to switch sessions.
- **Consistent Toolbar and Selector Tooltips ([QuickTooltip.swift](Quiper/QuickTooltip.swift), [MainWindowController.swift](Quiper/MainWindowController.swift), [CollapsibleSelector.swift](Quiper/CollapsibleSelector.swift))**: Standardized toolbar and selector tooltips, added readable shortcut hints, eliminated flicker during adjacent hover transitions, and preserved session titles when lazily restored tabs are reopened.
- **ChatGPT Template Compatibility ([Settings.swift](Quiper/Settings.swift))**: Refreshed ChatGPT's prompt input selector and History action for current page layouts, including the logged-out search experience.
- **Uninterrupted Template Validation ([Constants.swift](Quiper/Constants.swift), [App.swift](Quiper/App.swift), [UpdateManager.swift](Quiper/UpdateManager.swift))**: Prevented onboarding, update, and migration prompts from interrupting template-validation sessions and restored the Control-Space overlay fallback used during validation.
...
- **Stale Open-State in Expanded Session Selector After Close ([CollapsibleSelector.swift](Quiper/CollapsibleSelector.swift))**: Fixed the expanded session selector continuing to show a just-closed tab as open. `refreshInstantiationState()` now rebuilds `customInstantiatedStates` and `customLockedStates` on the expanded control before triggering redisplay, so the closed tab's indicator clears immediately without needing a collapse/expand cycle.
- **Sub-Window Shortcuts Intercepted by Main Window ([MainWindowController+InputHandling.swift](Quiper/MainWindowController+InputHandling.swift))**: Fixed Quiper swallowing keyboard shortcuts (e.g., `Cmd+W`) intended for an open sub-window (such as an image opened in a new window). The local event monitor's `handleCommandShortcut` path now respects the existing `hasModalWindow` / `skipModalCheck` guard — matching the behaviour already applied to `handleFlagsChanged` — so all interactions are passed through to the sub-window when it is key.
...
- **Persistent Prompt History ([Settings.swift](Quiper/Settings.swift), [SettingsView.swift](Quiper/SettingsView.swift), [SettingsModels.swift](Quiper/SettingsModels.swift), [MainWindowController.swift](Quiper/MainWindowController.swift), [MainWindowController+Selectors.swift](Quiper/MainWindowController+Selectors.swift), [MainWindowController+Actions.swift](Quiper/MainWindowController+Actions.swift))**: Added a configurable global setting to record prompt history per-session, along with a toolbar button (`clock.arrow.circlepath`) displaying a pop-up context menu to view and restore past prompts, toggle prompt history recording per-session, or clear history.
- **Headless macOS Code Signing & Apple Notarization ([.github/workflows/integration_delivery.yml](.github/workflows/integration_delivery.yml) & [build-app.sh](build-app.sh))**: Added robust support for cryptographically signing and notarizing builds natively via GitHub Actions.
  - **Dynamic Extraction**: Extracts signing identities and team IDs dynamically from your imported `.p12` certificate.
  - **Notarization & Stapling**: Automatically submits builds to Apple's Notarization Service (`xcrun notarytool`) and staples cryptographic tickets to the `.app` bundle, allowing users to run downloaded releases with a simple one-click confirmation.
  - **Hardened Runtime**: Integrates macOS Hardened Runtime (`--options runtime`) required for secure Apple distribution.
  - **Fallback Security**: Automatically and gracefully falls back to ad-hoc signing (`-`) and skips notarization with diagnostic logs on standard Pull Requests or local development builds.

### Changed

- **CI/CD Delivery Format ([.github/workflows/integration_delivery.yml](.github/workflows/integration_delivery.yml))**: Upgraded delivery artifacts for a premium distribution experience.
  - **DMG & ZIP Separation**: Separated the workflow output into independent, raw `.dmg` and `.zip` uploads, completely bypassing GitHub's default double-compression of release assets.
  - **Native Drag-to-Install**: Packages the application into a standard macOS DMG disk image (`.dmg`) using `hdiutil` in the delivery workflow, providing a native macOS drag-to-install experience.
...
- **Selector Lock Icon Synchronization ([MainWindowController+Actions.swift](Quiper/MainWindowController+Actions.swift))**: Fixed a bug where service selectors (both static and collapsible capsule modes) would fail to update their cached lock states when a secure engine was successfully unlocked or locked, causing them to display a stale lock icon. Added calls to `refreshServiceSegments()` during all lock and unlock flow transitions.
