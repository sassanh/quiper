...
- **Biometrics Focus-Wake & Auto-Reprime**: Resolved an issue where remaining locked for long periods caused the TouchID (`LAContext`) context to time out or become pale and unresponsive. Added active focus observers for `NSWindow.didBecomeKeyNotification` and `NSApplication.didBecomeActiveNotification` inside [LockOverlayView.swift](file:///Users/sassanharadji/Projects/Personal/quiper/Quiper/Components/LockOverlayView.swift) to automatically teardown stale contexts and re-prime a highly responsive biometric view on focus.
