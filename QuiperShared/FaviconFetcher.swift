...
/// Per-platform image processing needed by `FaviconFetcher` so the fetch pipeline
/// can stay in `QuiperShared`. Each app registers its implementation once at launch.
protocol FaviconImageProcessing {
    /// Converts raw image data to a base64 PNG string, downscaling oversized images
    /// to a 128px bounding box and rejecting images with no visible pixels.
    @MainActor static func encodePNG(data: Data) -> String?
    /// True when the base64 PNG is at least 96x96 pixels.
    @MainActor static func isHighRes(base64: String) -> Bool
}

/// Attempts to fetch a favicon for an engine URL. Tries HTML parsing, then
/// /favicon.ico, then the Google faviconV2 API, returning the best PNG as base64.
enum FaviconFetcher {
    static var imageProcessor: (any FaviconImageProcessing.Type)?

    static func configure(imageProcessor: (any FaviconImageProcessing.Type)?) {
    ...
    static func isHighRes(_ base64: String) -> Bool {
        imageProcessor?.isHighRes(base64: base64) ?? false
    }

    /// Encodes raw image data to a base64 PNG string using the configured processor.
    static func encodePNG(data: Data) -> String? {
        imageProcessor?.encodePNG(data: data)
    }
