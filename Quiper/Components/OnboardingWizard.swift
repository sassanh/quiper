...
struct OnboardingWizardView: View {
    ...
    private func runSetup() {
        ...
        Task {
            ...
            // 2. Migrate cookies from the default store to each selected engine
            if hasLegacyData && !selectedMigrationServices.isEmpty {
                statusText = "Reading legacy session cookies..."
                let legacyCookies = await defaultStore.httpCookieStore.allCookies()
                ...
                for serviceID in selectedMigrationServices {
                    if let service = settings.services.first(where: { $0.id == serviceID }) {
                        ...
                        let targetStore = WKWebsiteDataStore(forIdentifier: serviceID)
                        for cookie in legacyCookies {
                            await targetStore.httpCookieStore.setCookie(cookie)
                        }
                    }
                }
            }
