...
# Re-sign to preserve the entitlements generated by Xcode (base + custom).
# With Xcode 27 the base entitlements (network, device, Local Network) are
# generated from build settings; stripping them breaks Bonjour and mic.
if command -v codesign >/dev/null 2>&1; then
    echo "🔏 Re-signing (identity: $SIGNING_IDENTITY)..."
    if [ "$SIGNING_IDENTITY" = "-" ]; then
        codesign --force --deep --sign - "$APP_NAME.app"
    else
        codesign --force --deep --options runtime --timestamp --sign "$SIGNING_IDENTITY" "$APP_NAME.app"
    fi
fi
